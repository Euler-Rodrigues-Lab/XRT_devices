...
def deserialize_pose_data(data: bytes, z_up: bool = True) -> list[Bone]:
    ...
    if not isinstance(data, (bytes, bytearray, memoryview)) or len(data) < 4:
        raise ValueError("pose packet must contain a bone count")
    count = struct.unpack_from("<i", data)[0]
    if count < 0 or count > 256 or len(data) != 4 + 32 * count:
        raise ValueError("invalid pose packet length or bone count")
    seen = set()
    ...
    try:
        ...
        for _ in range(bone_count):
            ...
            bone_id = bone_data[0]
            if bone_id in seen or not all(math.isfinite(v) for v in bone_data[1:]):
                raise ValueError("duplicate bone ID or non-finite pose")
            if sum(v * v for v in bone_data[4:]) < 1e-12:
