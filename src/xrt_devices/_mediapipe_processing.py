...
class MediaPipeTeleopDevice:
    ...
    def __init__(self, camera_id=0, debug=False, mirror_actions=False, env=None,
                 *, pose_model, hand_model, display=False):
        """
        ...
        global cv2
        import cv2
        from ._mediapipe_tasks import make_api
        mp = make_api(pose_model, hand_model)
        ...
        self.controller_state_lock = threading.RLock()
        ...
        while not self.stop_event.is_set():
            try:
                ...
                with self.controller_state_lock:
                    ...
                    self.human_wrist_poses = {"left": None, "right": None}
                    self.human_hand_poses = {
                        side: {"landmarks": None, "confidence": 0.0}
                        for side in ("left", "right")
                    }
                    self._process_hand_landmarks(hand_results, pose_results, frame)
                    self._last_received = captured_at
                    self._sequence += 1
        ...
        if self.controller_state_lock.acquire(timeout=0.1):
            try:
                ...
                for arm_side in ["left", "right"]:
                    ...
                    # Add torso rotation
                    controller_state["R_torso"] = controller_state['sew_poses']['R_world_body']
    ...
    def input2action(self, mirror_actions=False):
        ...
        # Handle torso rotation if available
        if state is not None and "R_torso" in state and state["R_torso"] is not None:
            ac_dict["R_torso"] = state["R_torso"]
