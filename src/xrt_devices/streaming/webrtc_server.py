...
class WebRTCServer:
    ...
    async def _create_offer_handler(self, request: Request):
        ...
        if self.pcs:
            return JSONResponse(status_code=409, content={"error": "one headset per device"})
        ...
        def on_datachannel(channel):
            label = channel.label
            self.channels[pc][label] = channel
        ...
        async def on_connectionstatechange():
            ...
            if self.connection_callback:
                self.connection_callback(pc.connectionState)
            if pc.connectionState in ("failed", "closed", "disconnected"):
                await pc.close()
                self.pcs.discard(pc)
                self.channels.pop(pc, None)
    ...
    def run(self):
        ...
        self._uvicorn = uvicorn.Server(uvicorn.Config(self.app, host=self.host, port=self.port))
        self._uvicorn.run()
    ...
    def send(self, label, payload):
        """Call on the asyncio server loop. Return number of peers sent to."""
        sent = 0
        for channels in list(self.channels.values()):
