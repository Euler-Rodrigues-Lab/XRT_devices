...
## Input conventions: SEED bones, IOBT, BVH, and MediaPipe

These tables compare **source keypoint definitions** and **the keypoints consumed
by processing**. Body-frame configuration is documented separately below. BVH playback uses the original repository's LaFAN mapping by
default, with explicit overrides for other joint names, units, and world axes.
“SEED” means the supported bone CSV/CSV.GZ export, including the
recording used by WARP; it does not mean arbitrary raw BVH or a different SEED exporter.
...
| Property | SEED bone CSV export | Live IOBT through XRT-Client | BVH (LaFAN defaults) | MediaPipe Pose + Hand Tasks |
|---|---|---|---|---|
...
| Conversion on replay | Bone rows are already converted: no second Unity-to-FLU transform | A CSV recorded by XRT_devices stores decoded internal bones; replay follows the same rule as SEED CSV | Scale and world rotation applied once while evaluating the hierarchy | Hand points are made wrist-relative and attached to the matching Pose wrist before body/wrist transforms |
...
paired IDs are **left / right**. SEED exports and IOBT share this **identifier
namespace**, so it is combined here. A shared ID does not prove identical source
anatomy, point availability, or exporter placement. The original VMD converter
maps `肩.L/R` to `{Left,Right}Shoulder`, `腕.L/R` to `{Left,Right}ArmUpper`,
`ひじ.L/R` to `{Left,Right}ArmLower`, and `手首.L/R` to
`{Left,Right}HandWrist`; those names should not be replaced by solver quantities.

| Source keypoint | SEED / IOBT `FullBodyBoneId` | BVH: original LaFAN mapping | MediaPipe Pose |
|---|---|---|---|
...
The SEED recording used by WARP omits IDs **5, 9, 12, 14, 17, 18, 44, 72, 74, 75,
79, 81, 82**. The original VMD exporter can also synthesize foot points; availability
must be checked per export. `FullBody_End(84)` is a sentinel, not a keypoint.
...
mapping entries do not imply that every LaFAN file contains those joints. Resolve
...
SEED and IOBT use the same body-keypoint selections, so they share one column.
...
| Processing keypoint | SEED / IOBT | BVH (LaFAN mapping) | MediaPipe |
|---|---|---|---|
...
frame**, following the active computation in the original
[`xr_robot_teleop_client.py`](https://github.com/kczttm/SEW-Geometric-Teleop/blob/main/projects/shared_devices/xr_robot_teleop_client.py#L113):
...
Live IOBT, SEED CSV replay, and BVH all use the upper-arm convention.
...
The following are **raw enum identifiers**, before the legacy SEED mapping and
fingertip adjustment described next. They describe the shared namespace, not a
guarantee that SEED and IOBT store the same anatomical point at every ID. XR finger
...
| Landmark chain | FullBody IDs, left / right (SEED and IOBT namespace) | BVH: original LaFAN mapping | MediaPipe Hand indices |
|---|---|---|---|
...
The original VMD converter exposes `root`, `tip`, and `mediapipe` mappings.
For example, `人指１.L` maps to `LeftHandIndexIntermediate(26)` in `root`
mode but `LeftHandIndexProximal(25)` in `tip` mode. `親指１.L` maps to
`LeftHandThumbProximal(21)` in `root` mode but
`LeftHandThumbMetacarpal(20)` in `tip` mode. Right-hand mappings change the
same way. Thus **combine the enum namespace, but retain the source-specific
finger definitions**; the CSV IDs alone do not identify the original export mode.
The LaFAN mapping has no fingers, so it cannot supply the palm geometry used by
...
Original definitions: [VMD exporter mappings](https://github.com/yunho-c/XR-Robot-Teleop-Server/blob/main/misc/visualize_vmd_body_pose.py)
(`get_active_mapping`) and [LaFAN reader and joint mapping](https://github.com/kczttm/SEW-Geometric-Teleop/blob/main/projects/shared_devices/offline_lafan_bvh_reader.py)
(`BVH_TO_FULL_BODY`). These are distinct from the derived solver slots below.
...
| Property | SEED bones | IOBT bones | BVH wrists | MediaPipe hands |
|---|---|---|---|---|
...
| Missing finger data | No valid coordinates yields an empty finger dictionary; missing slots in a partially valid finger are zero-filled by the legacy mapper | Same | No finger payload from the LaFAN mapping | Missing/low-confidence/invalid hand geometry yields no fresh finger dictionary; fingers are not held over dropouts |
...
The default LaFAN mapping supplies no fingers or gripper values. The skeleton
